...
describe("Export Component", () => {
  ...
  const mockDiaryNotes = {
    ...
    "2025-01-02": {
      values: [{ id: "note3", value: "Work meeting was stressful but manageable" }],
    },
    "2025-01-03": {
      values: [
    ...
    "2025-01-06": {
      values: [{ id: "note9", value: "Therapy insights: working on coping strategies" }],
    },
  };
  ...
  const renderWithContext = (component) => {
    return render(
      <DiaryDataContext.Provider value={[mockDiaryData, jest.fn()]}>
        <DiaryNotesContext.Provider value={[mockDiaryNotes, jest.fn()]}>{component}</DiaryNotesContext.Provider>
  ...
  describe("onPress Action Tests", () => {
    ...
    test("should call shareAsync after file generation", async () => {
      ...
      await waitFor(() => {
        expect(Sharing.shareAsync).toHaveBeenCalledWith("/path/to/generated/file.pdf", { UTI: ".pdf", mimeType: "application/pdf" });
      });
    });
  ...
  describe("File Saving with Custom Name Tests", () => {
    ...
    test("should share file with custom name", async () => {
      ...
      await waitFor(() => {
        expect(Sharing.shareAsync).toHaveBeenCalledWith("/path/to/generated/Rapport janvier 2025.pdf", { UTI: ".pdf", mimeType: "application/pdf" });
      });
    });
  ...
  describe("Integration Tests", () => {
    ...
    test("should complete full user flow: input name → press button → generate file with custom name", async () => {
      ...
      await waitFor(() => {
        ...
        expect(Sharing.shareAsync).toHaveBeenCalledWith("/path/to/generated/Test Integration.pdf", { UTI: ".pdf", mimeType: "application/pdf" });
      });
    });
