  ...
  const render = (beck, beckId) => {
    return (
      <View style={styles.posologyItem}>
        <View style={styles.posologyName}>
          {beck?.mainEmotion && beck?.mainEmotionIntensity ? (
            <>
              ...
              {beck?.where ? <Text style={styles.text2}>{beck?.where}</Text> : null}
            </>
          ) : (
        ...
        <TouchableOpacity onPress={() => handleDelete({ date: beck?.date, beckId })}>
  ...
  return (
    <View>
      ...
      {Object.keys(data).map((i, j) => {
        const beck = data[i];
        return (
          <TouchableOpacity key={j} style={[styles.container]} onPress={() => onPress(beck, i)}>
