...
import { INDICATEURS_SOMMEIL, INDICATEURS_LISTE_ONBOARDING_SOMMEIL } from "../../../utils/liste_indicateurs.1";
...
export const OnboardingSleep = ({ navigation }) => {
  ...
  useFocusEffect(
    useCallback(() => {
      ...
      if (!userIndicateurs?.some((_ind) => _ind.uuid === INDICATEURS_SOMMEIL.uuid)) setToggleIndicateur(INDICATEURS_SOMMEIL);

      // deplier par defaut si au moins un des enfants est selectionné
      if (userIndicateurs?.some((_ind) => INDICATEURS_LISTE_ONBOARDING_SOMMEIL.map((e) => e.uuid).includes(_ind.uuid) && _ind.active)) {
      ...
      <ScrollView keyboardShouldPersistTaps="handled" style={onboardingStyles.scroll} contentContainerStyle={onboardingStyles.scrollContentContainer}>
        <View style={onboardingStyles.container}>
          ...
          <View style={onboardingStyles.imageContainer} key="image">
            <Image source={require("../../../../assets/imgs/onboarding/sleep.png")} style={onboardingStyles.imageSize} />
          </View>
          <TouchableOpacity
            key="main-checkbox"
            style={[
              stylesA.choixContainer,
              userIndicateurs?.find((_ind) => _ind.uuid === INDICATEURS_SOMMEIL.uuid && _ind.active) ? stylesA.choixContainerSelected : null,
            ]}
          ...
          {isSleepTroubleEnable ? (
            <View key="secondary-details">
              ...
              <CheckBoxList list={INDICATEURS_LISTE_ONBOARDING_SOMMEIL} userIndicateurs={userIndicateurs} setToggleIndicateur={setToggleIndicateur} />
            </View>
          ) : null}
        </View>
      </ScrollView>
