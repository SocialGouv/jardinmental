    ...
    <SafeAreaViewWithOptionalHeader style={onboardingStyles.safe}>
      ...
      <ScrollView keyboardShouldPersistTaps="handled" style={onboardingStyles.scroll} contentContainerStyle={onboardingStyles.scrollContentContainer}>
        <View style={onboardingStyles.container}>
          ...
          {INDICATEURS_LISTE_ONBOARDING_CUSTOM_SIMPLE.map((group, index) => (
            <CheckBoxList key={index} list={group} userIndicateurs={userIndicateurs} setToggleIndicateur={setToggleIndicateur} />
          ))}
        </View>
      </ScrollView>
        ...
        return (
          <TouchableOpacity key={index} style={[stylesA.choixContainer, isActive ? stylesA.choixContainerSelected : null]} onPress={() => setToggleIndicateur(_indicateur)}>
            {isActive ? (
              ...
              <View>
                <RoundButtonIcon backgroundColor="#f4f4f4" iconColor="#e1e1e1" borderWidth={0.5} borderColor="#e1e1e1" icon="validate" visible={true} medium />
              </View>
            )}
