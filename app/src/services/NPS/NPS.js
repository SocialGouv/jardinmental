...
import {Dimensions, Modal, Platform, AppState, Alert, StyleSheet, View, ScrollView, KeyboardAvoidingView, TextInput, TouchableOpacity, Keyboard} from 'react-native';
  ...
  renderFirstPage() {
    ...
    return (
      <>
        ...
        <Mark selected={useful} onPress={this.setUseful} bad={getCaption('feedback.rate-app.useful.not')} good={getCaption('feedback.rate-app.useful.extremely')} />
        <Text style={styles.topSubTitle}>{getCaption('feedback.rate-app.probable')}</Text>
        <Mark selected={reco} onPress={this.setReco} bad={getCaption('feedback.rate-app.probable.not')} good={getCaption('feedback.rate-app.probable.extremely')} />
...
const captions = {
  ...
  'feedback.contact.description': 'Echanger avec vous serait précieux pour améliorer notre service, laissez-nous votre adresse mail si vous le souhaitez.',
