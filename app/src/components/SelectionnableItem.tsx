...
export const SelectionnableRadioItem = ({
  onPress,
  selected,
  children,
  text
  ...
  >
    <View className="flex-row items-center mb-4">
      {selected ? (
        <View
          className={mergeClassNames("w-6 h-6 items-center justify-center bg-primary mr-2", "rounded-full")}
        >
          <View className="bg-white w-2 h-2 rounded-xl"/>
        </View>
      ) : (
        <View
          className={mergeClassNames(
            "w-6 h-6 items-center justify-center border-2 border-gray-300 mr-2",
            "rounded-full"
          )}
        >
          <Text className="text-white text-xs" />
