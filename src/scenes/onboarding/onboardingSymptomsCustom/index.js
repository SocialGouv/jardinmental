...
const CustomSymptomScreen = ({navigation, route, settings = false}) => {
  ...
  useEffect(() => {
    ...
    (async () => {
      const customSymptomsKeys = Object.keys(chosenCategories).filter(
        (e) => !displayedCategories[e],
      );
      const defaultSymptomsKeys = Object.keys(chosenCategories).filter(
        (e) => !!displayedCategories[e],
      );
  ...
  const removeSymptom = async (value) =>
    setChosenCategories({...chosenCategories, [value]: false});
      ...
      <ScrollView
        ...
        {!settings && (
          <View style={styles.buttonWrapper}>
            ...
            <Button
              title="Valider"
              onPress={() => navigation.navigate('onboarding-drugs')}
            />
          </View>
        )}
      </ScrollView>
